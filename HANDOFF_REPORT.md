...
- **11:47/11:50 对照，已同步到本地 `logs/`。要点：这两次都不是本项目跑的 pick**，`pick_object` 全程走旧
  ...
  **结论：本项目规划器并没有"抓不到"，是 11:03 时目标刚好在可解边界之外。**
  ...
  - **MuJoCo 仿真复刻两个位置，结果与现场逐一吻合**（2026-08-02，`--rest-bottle-on-table`，左臂，
    命令见"常用命令"）：11:50 位 `[0.4778,0.0666,0.0564]` → **抓取成功**，两指垫双边接触
    `water_bottle_body`，**物理抬升 91.4 mm**，41 点路径，产物 `outputs/sim_1150_success/`；
    11:03 位 `[0.5093,0.0856,0.0517]` → **失败**，`planning_failed / no continuous IK path`，
    0 点路径。**独立物理引擎给出同样的成败分界。** 同一次运行里本项目 TRAC-IK 与 MuJoCo FK 交叉
    审计 641 采样**最大位置误差 4.0e-07 m**，两臂 `passed`——第三次独立证伪上面的 FK 不一致结论。
    已知偏差：Oracle 把瓶心 z 估到 0.0839（实摆 0.0564）**偏高 27.5 mm**，即"仿真瓶模型含瓶颈"，
    成败结论不受影响但不可当绝对精度用。
