...
- **`search_object` 服务分派已入库（2026-08-03，修掉一处跨仓同步风险）**。父项目**已提交**的
  ...
  诊断图路径回传，父项目 `_normalize_search_result` 早已在读这四个键，此前恒为 `None`。**服务器尚未同步**：
  thor 的 `main` 仍停在 `e7cb2e4`、工作区留着那 51 行（已逐字节核对与本提交一致，可安全丢弃），因 `git push`
  与 `scp` 被本机权限策略拦截，需人工推送后在 thor 执行 `git stash && git fetch && git merge --ff-only`。
