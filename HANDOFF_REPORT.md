...
- 现场缓存图复核（25 mm 棋盘单目 PnP 深度 0.87 m 与防错锁整板视差
  0.85 m 互证）：瓶子真实机身坐标约 `[0.70,-0.04,-0.10] m`，距双肩约
  `0.78 m`，远超臂+TCP 上限；旧标定链路系统性低估距离约 15–20%。
  SGBM 在棋盘/地毯等周期纹理会锁错周期，不可用于标定验收。
- 仿真仓库已同步新标定与新外参（repo 与 macos 两份 yaml）；仿真 16 项
  测试通过，`x=0.48` 右臂全物理抓取冒烟通过（感知误差 7 mm）。
- CaP-X 旧后端对照：IK 末端定义为腕前 `0.05 m` 加经验 TCP 修正（残差
  19–43 mm），臂展预检 `0.70 m` 仅为宽松过滤；其外参/TCP 数值与本项目
  定义不兼容，不可直接搬用。
- 对照分支 `test/capx-body`（腕前 0.05 m TCP + legacy 标定/外参）仿真
  实证：桌面图原生距离 0.554 m 全候选 IK 无解（该语义可达仅约 0.46 m）；
  近距 0.38 m 规划 42 点通过，但真实 15 cm 手指超伸 10 cm，插入时把瓶子
  撞下桌、零双指接触。其参数只在自身误差相消的现场闭环内可用。该路线
  已放弃，分支仅作记录保留。
- 新标定已在 Thor 真实模型链路（CREStereo/YOLO-World/SAM2）用桌面缓存图
  ...
  与本机 SGBM 近似深度 `0.6845 m` 相差 3.6 mm，互为交叉验证。
- 该样本瓶心距双肩约 `0.68 m`，超串联上限 `0.5606 m` 约 12 cm；任何外参
  微调都不能使其可达，此场景现场必须先由底盘逼近。
