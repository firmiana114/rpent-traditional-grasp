...
- **2026-08-02 16:42：夹爪首次真正夹住瓶子。** 上层审计写 `failure_unverified`，但那是被下游到位判据
  卡住，**抓取本身成功**：执行顺序为 张手 → 走到 `grasp`(index 21) → `close_gripper(require_contact=True)`
  → 抬升 → 后撤；**本次没有 `close_gripper failed`**（15:35 空抓那次有），而闭合无接触必抛异常，
  故**夹爪确实合上并检测到持续接触**。失败在 **index=24 = `lift:3/10`**，已在 grasp 之后。
  经验偏置已生效（本次选右臂，目标由 `[0.4750,0.0705,0.0431]` 移到 `[0.4414,0.0969,0.0898]`）。
  - **卡住的原因是 CapX 到位判据的死区**：正常到位要 `误差<=0.03 且 速度<=0.05`；受阻到位要
    `速度<=0.10 且 误差>=0.05 且 力矩偏差>=1.5Nm`。实测 `误差 0.0342 / 速度 0.0123`，落在
    **0.03 与 0.05 之间**，两条判据都不满足，只能干等 5 s 超时。速度说明手臂已静止，只是稳态位置有
    偏差——夹住瓶子后负载增加，而 `sol_tauff` 的重力前馈按空臂算，PD 必然留下 `稳态误差=负载力矩/kp`。
    **属父项目/CapX 侧，不是规划或几何问题。**
- **"缓慢下放后突然举起双手"已定位到 CapX 保活线程**（2026-08-02 现场观察）。
  `_keepalive_loop` 以 30 Hz **重发 `_hold_q`——最后一次指令目标，不是当前实测位置**，且
  `_set_keepalive` 只是翻一个布尔量、**恢复时不做任何插值**。`_home_q = np.zeros(14)`，而零位正是
  "抬起并前伸"。于是：**DDS 指令流一旦中断，双臂在重力下缓慢下垂；保活一恢复就把下垂前的目标当作
  一个阶跃发出去，双臂瞬间弹回零位**——观察到的现象与此逐条吻合，且解释了为什么是"双手"（`_hold_q`
  是 14 维全臂向量）。
  - **这与种子漂移是同一根因**：漂移正是在这段下垂窗口里被测到的（14:18 那次 0.724→0.672 rad 且在
    收敛，就是被拉回的过程；16:42 首次尝试也复发 `max_drift=0.744`）。此前记的"GIL 饿死保活线程"
    推测由此得到现场佐证：`_get_collision_checker()` 在 **arm_service 同一进程**内构建 pinocchio 模型
    与 1257 个碰撞对耗时 4 s，期间 C++ 调用持 GIL，30 Hz 保活线程被饿死。
  - **建议修法（属父项目/CapX，未实施）**：① 保活恢复时先把 `_hold_q` 重置为**当前实测位置**再平滑
    过渡到目标，杜绝阶跃；② 把碰撞检查器改为服务启动时一次性构建，或移出 arm_service 进程；
    ③ 抓取后段放宽到位容差或让 `sol_tauff` 计入负载，消除上面那个死区。

- **2026-08-02 15:28/15:35 首次真机执行：空抓并推开瓶子**（已被 16:42 取代，保留结论）。夹爪自报
  `empty-close position without sustained contact`，安全链正确中止。**自碰撞预筛按预期奏效**：两次分别
  丢弃 25/24 个碰撞候选，`rank=1` 一次通过，`max_seed_drift_rad` 仅 0.0004/0.001 rad。
  **"阶跃参考"假说已排除**：规划侧 33 点相邻位移 0.068–0.135 rad 连续无跳变、FK 反算与规划目标逐位
  相同；下发侧 `move_arm:797` 做 smoothstep 插值（30 Hz）并逐点等收敛。留意 `duration=0.12` 只产生
  3 个插值子步（最大单拍 0.065 rad/33 ms）、33 点各自加减速导致顿挫、收敛容差 0.03 rad 在 0.5 m 臂展
  上约合 15 mm 落点散布——15 mm 不足以解释 87 mm 偏差，故当时判定几何偏置为主嫌，16:42 已证实。
- **经验抓取偏置已接入（2026-08-02，用户决定先试）**：`left/right_empirical_grasp_offset_m`，
  ...
  - **已扣除两条链路的瓶心定义差**（2026-08-02，用户决定）。该标定的真值是 legacy 的
    `median(整个掩码点云)`，而掩码只看得见前半边，中位数落在靠近前表面处；本项目取商标带鲁棒
    前表面深度再**推半个瓶径**到几何中心。在 11:03 与 11:50 两帧实测两者差 **24.6 mm**，且分解
    完全一致：沿接近 **+17.9 mm**、侧向 +2.5 mm、竖直 **-16.6 mm**（我方更靠里、更低；legacy 的
    中位数被瓶盖瓶颈拉高）。直接套用等于把定义差当成夹爪误差，故已扣除，配置值改为左臂
    `[-0.0033,+0.0849,+0.0179]`、右臂 `[-0.0226,+0.0363,+0.0467]` m。
  - **关键分野**：轴向与竖直分量确实是 legacy 感知定义的产物、不可直接迁移；但**侧向的 87 mm
    几乎不受影响**（只差 2.5 mm），两条链路在侧向上一致。故侧向要么是真实夹爪几何偏置，要么是
    **两链共有**的侧向偏差（如共用的相机-机身外参偏航），它正对应现场目视的“夹爪停在瓶子右侧”。
  - **前提**：24.6 mm 是在同一瓶、相近视角的两帧上测的；遥操标定用的是 `VLA_record4` 的其他场景，
    定义差未必仍是 24.6 mm。两帧结果一致是好迹象，但**摆位样本仍只有一种**。
  - **仍应做的**：用尺直接量夹爪两指内表面中点相对腕部的实际位置（轴向+侧向）。这是唯一**不经过
    感知链**、能把 TCP 误差单独摘出来的办法；若尺子证明 TCP 本来就对，则侧向那 87 mm 属感知误差，
    该去修相机外参而不是补 TCP。
...
- **11:03 / 11:47 / 11:50 三次已结案**（原始数据在 gitignore 的 `logs/`）。11:47、11:50 的 pick 都由旧
  Contact-GraspNet 承担，本项目只做 `search_object`；11:03 是唯一由本项目 pick 的，失败原因是目标刚好
  在可解边界之外。结论：① **离线复算与 MuJoCo 仿真在三个位置的成败与现场逐一吻合**——11:50 位
  `[0.4778,0.0666,0.0564]`（左肩距 0.5121）与 11:47 位（0.5099/0.5114）仿真均抓取成功（双指接触、抬升
  91.4–91.5 mm，产物 `outputs/sim_1150_success/`、`sim_1147/`），11:03 位（0.5422）离线与仿真均失败；
  **只差 30 mm 跨过边界，独立物理引擎给出同一分界**；同次运行的 TRAC-IK 与 MuJoCo FK 交叉审计 641 采样
  最大位置误差 **4.0e-07 m**。② **11:47 崩溃 100% 源于坏 bbox 触发硬失败**：LLM 目测的
  `[268,204,312,294]` 落在空台面上（已叠图确认），几何门禁按设计拒绝，但本项目**抛异常而非回退**；
  而 11:50 的 LLM 调了不带 bbox 的 `search_object`，**促成对方抓取成功的 bbox 正是本项目给的**。
  ③ **“把仿真的肩后收搬到实机”已实测否决**：11:03 帧肩俯仰 0→0.80 rad 每档 24 候选全无解，关节限位内
  随机多起点 60 个种子 0 次成功（11:50 帧 60 个中 8 次成功且零位在内），故种子只影响收敛、不改变解的
  存在性，**零位本身已是好种子**；生产链路根本没有准备姿态，`prepare_arm` 只初始化控制器而构造即回零。
  ④ 仿真 viewer 开头“先动一下再停”是脚本的固定准备段加约 10 s 规划阻塞，非异常；**姿态网格明显不
  对称**，只有正俯角有解，一半候选是无效开销；11:50 的 3 次 0.1 m 前进原语**实际没移动底盘**。
- **早期现场运行留档**：11:03（`logs/20260802-11:03:28...`）瓶心 `[0.5093,0.0856,0.0517]`、深度 0.5658 m、瓶径 0.0587 m，左肩距 0.5422 m 超实测规划半径 2.2 mm，24 个候选全 `no IK solution`；0801（`logs/20260801-16:48:44...`）旧标定 `depth=0.607m diameter=0.062m`，底盘全程未动，30 s 内深度由 0.607 变 0.527 m 是画面里的人把桌子推近约 80 mm。**瓶径真值仍存疑、暂缓**：现场量得瓶底 55.0 mm 且称昨今同一瓶（与此前 62 mm 冲突），而本项目估的是中部商标带（掩码中部 38 px、瓶底 33 px，下收 13%），同一深度图量瓶底得 57.9 mm；**要定案需现场补量瓶身最宽处直径与瓶高**，在此之前不要用瓶径反推深度尺度。
