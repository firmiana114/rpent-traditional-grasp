...
- **三次运行的 pick 后端归属（命名极易混淆，务必先看这条）**：父项目 `wuxi_adapter.py:421`
  `_pick_object_legacy` 的错误串字面写着 `traditional grasp detector did not return a grasp message`，那是
  **Contact-GraspNet 时代的旧叫法**，比本项目还早；总结 JSON 里 LLM 又自己写了 `"backend": "air_robot
  traditional method stack"`。判别只能靠**唯一串**计数（依次为 `did not return a grasp message` /
  `contact_graspnet` / `required_base_advance_m` / 收到 `plan_pick`）：**11:03 = 0/0/2/1 → 本项目
  traditional-live**；11:47 = 2/0/0/0 → legacy，失败；11:50 = 2/2/0/0 → legacy，成功。

- **父项目从不记录当前 pick 后端模式**（`run.log`、`observe_scene` 的 `capabilities`/`roots` 里都没有），
  这是归属只能靠反推错误串的根本原因，也导致测试人员与日志证据分歧（测试人员认为 11:47 跑的是本项目）。
  **建议父项目在 `wuxi_adapter._pick_backend_mode()` 落一条 INFO，并把 mode 加进 `observe_scene` 输出。**
  机制上无歧义：默认值是 `traditional-live`，进 legacy **必须显式 export**；分发是 `if mode != "legacy"`
  二选一，`traditional_grasp_backend.py` **不含任何回退 legacy 的路径**；错误串全仓库仅一处；`AIR_ROBOT_PICK_BACKEND` 未见于任何脚本或 `.bashrc`，故最可能是手工 export 后残留。
- **11:47/11:50 对照，已同步到本地 `logs/`。要点：这两次都不是本项目跑的 pick**，`pick_object` 全程走旧 Contact-GraspNet，本项目只承担 `search_object`（11:47 两次、11:50 七次）；**故现场尚未做过同场景的跨方法 A/B**，要做需在当前摆位下用 `AIR_ROBOT_PICK_BACKEND=traditional-live` 再跑一次。**结论：本项目规划器并没有"抓不到"，是 11:03 时目标刚好在可解边界之外。**
  - **离线复算 + MuJoCo 仿真，三个位置的成败与现场逐一吻合**（`--rest-bottle-on-table`，左臂，命令见
    "常用命令"）。11:50 位 `[0.4778,0.0666,0.0564]`（左肩距 0.5121）：离线规划成功（`within_radius=True`、
    24 候选中 `pitch_+10/+20/+30` 有解、38 点、停在 `motion_gated`），仿真**抓取成功**——双指接触
    `water_bottle_body`、**抬升 91.4 mm**、41 点，产物 `outputs/sim_1150_success/`。11:03 位
    `[0.5093,0.0856,0.0517]`（0.5422）：两者**均失败**，`planning_failed / no continuous IK path`。
    **两个位置只差 30 mm，恰好跨过可解边界，独立物理引擎给出同一分界。** 同次运行的本项目 TRAC-IK
    与 MuJoCo FK 交叉审计 641 采样**最大位置误差 4.0e-07 m**，两臂 `passed`——第三次独立证伪上面的
    FK 不一致结论。已知偏差：Oracle 把瓶心 z 估高 **27.5 mm**（"仿真瓶模型含瓶颈"），成败结论不受
    影响但不可当绝对精度用。
  - **11:47 那帧仿真也抓到了，坐实其失败与可达性无关。** 事后用自带检测器复算两帧，瓶心
    `[0.4760,0.0664,0.0580]` / `[0.4772,0.0665,0.0569]`，左肩距 **0.5099 / 0.5114 m**（与 11:50 的
    0.5121 m 几乎相同，同场景相隔 3 分钟），两帧送仿真**均抓取成功**（抬升 91.5 / 91.4 mm，双指接触，
    41 / 40 点）。**故 11:47 崩溃 100% 源于坏 bbox 触发硬失败。**
  - **"把仿真的肩后收搬到实机"已实测否决。** 种子就是规划时刻的真实关节角（`service.py` 把 `current_q`
    喂给 `update_joint_state`，`api.py:602` 取作 TRAC-IK 种子），技术上只要规划前摆好姿态即可，**但没
    用**：11:03 帧把肩俯仰从 0 扫到 0.80 rad（0°→45.8°），**每一档 24 个候选全部无解**；再做关节限位内
    **随机多起点**（TRAC-IK 是局部解算器，多起点才能判定解是否存在），11:03 帧 **60 个种子 0 次成功**，
    11:50 帧 **60 个种子 8 次成功且零位在内**。结论：11:03 的目标不在灵巧工作空间内，任何准备姿态都救不
    了；且**零位本身已是好种子**（随机姿态多数更差），换准备姿态反有变坏风险。另注：生产链路**根本没有
    准备姿态**——`client.py:700` 的 `prepare_arm` 只初始化 CapX 控制器，而构造即自动回零。
  - **看 viewer 时的"先动一下再停"不是异常**：开头是仿真脚本的 `prepare_fixed_ready_pose`（与瓶子位姿
    无关的固定准备段）——先把 `shoulder_pitch` **+0.35 rad**（TCP 后收 90.6 mm、下降 103.6 mm，纯转动、
    肩距不变 0.4043 m）1.0 s，再以此为 IK 种子解到固定准备 TCP 1.5 s；随后**静止约 10 s 是规划阻塞**
    （可行候选每个约 0.63 s 做 29 点连续逆解，被拒的约 0.02 s），物理不推进；最后才是 40 点抓取轨迹。
    另：**姿态网格明显不对称**，只有正俯角有解，负俯角与偏航全灭，一半候选是无效开销。
  - **底盘实际上没动**：11:50 里 3 次 0.1 m 前进原语都报成功，但前后两次 `search_object` 测得的瓶心只变
    了约 1.5–6 mm，`approach_object` 最终报 `max_iterations reached`，故成功**不能**归因于底盘前移。
    （该轮未记录底盘里程，此判断基于目标距离而非位姿读数。）
  - **11:47 与 11:50 的真实差别是"LLM 有没有让本项目的检测器跑"。** Contact-GraspNet 自带检测器在两次
    ...
    `[203,194,252,299]` 正是本项目给出的**；11:47 的 LLM 则只喂自己目测的框，那个 `[268,204,312,294]`
    落在**空台面**上（已叠图确认），几何门禁按设计拒绝（`瓶径越界 0.1608m`、第二次
    `商标带深度离散过大 mad=0.0284m`），但本项目**抛异常**而非回退，`approach_object` 连带失败，
    重试门禁锁死。**显式 bbox 被拒时应回退到自带检测器**。
  - 与 Contact-GraspNet 的结构差异：本项目要求 TCP **精确落在瓶心**、姿态取自**固定 ±30° 网格**，24 个全无解即整体失败；对方从 3267 点点云**生成连续候选**并排序（本次取 `candidate_id=2`）。**三条改进（按价值排序）**：① 显式 bbox 被几何门禁拒绝时回退到自带检测器；② 允许 TCP 落在瓶心**之前**（抓取深度参数），回收 10–20 mm 即等量延长可达距离；③ 姿态网格向有解一侧加密。
- **2026-08-02 11:03 现场运行（旧标定生效后的第一次，仍失败；三次里唯一由本项目承担 pick 的）**，已同步
  到 `logs/20260802-11:03:28_air_robot_task_s0`（`logs/` 已 gitignore）。场景已换（瓶子在白色矮柜台面上）：
  - 瓶心 `[0.5093,0.0856,0.0517]`、深度 `0.5658 m`、瓶径 `0.0587 m`、MAD 4.1 mm；本机复现与 Thor 差
    **< 0.3 mm**。左肩距 `0.5422 m`：在杆长上界内（0.5606），只比实测规划半径 0.540 超 **2.2 mm**。种子
    关节角 14 维全部 `|q| < 0.015 rad`。**左右臂 24 个姿态候选全部 `no IK solution`**——失败在目标位姿。
