...
- **2026-08-02 16:42：夹爪首次真正夹住瓶子。** 上层审计写 `failure_unverified`，但那是被下游到位判据卡住，
  ...
  **本次没有 `close_gripper failed`**（15:35 空抓那次有），而闭合无接触必抛异常，故**夹爪确实合上并检测到
  持续接触**；失败在 **index=24 = `lift:3/10`**，已在 grasp 之后。经验偏置已生效（选右臂）。**卡住原因是
  CapX 到位判据的死区**：正常到位要 `误差<=0.03 且 速度<=0.05`，受阻到位要 `速度<=0.10 且 误差>=0.05 且
  力矩偏差>=1.5Nm`，实测 `误差 0.0342 / 速度 0.0123` **落在两者之间**，只能干等 5 s 超时——手臂已静止只是
  稳态位置有偏差（夹住瓶子后负载增加而 `sol_tauff` 按空臂算）。**属父项目/CapX 侧，不是规划或几何问题。**
- **"缓慢下放后突然举起双手"已定位到 CapX 保活线程**（2026-08-02 现场观察）。`_keepalive_loop` 以 30 Hz
  **重发 `_hold_q`——最后一次指令目标，不是实测位置**，恢复时不插值，而 `_home_q` 为零位（"抬起前伸"）。
  ...
  1257 个碰撞对耗时 4 s、持 GIL 饿死保活线程。**修法（属父项目/CapX）**：① 保活恢复先重置 `_hold_q` 为实测
  位置再平滑过渡（**已实施但对本场景无效，见下条**）；② 碰撞检查器移出该进程或启动时构建（**未修，已证实
  是主因**）；③ 放宽后段到位容差或让 `sol_tauff` 计入负载（**未修**）。
- **保活阶跃补丁守错了路径，2026-08-03 10:24 现场复现（首次真机验证，结论是"未修复"）**。补丁确在
  `cap-x/capx/arm/controller.py:633`，但 `_ramp_hold_to_target()` 只在 `_set_keepalive(False→True)` 这一**状态
  翻转**时调用；GIL 饿死期间 `_keepalive_enabled` **始终为 True、根本不经过 `_set_keepalive`**，线程只是没被
  调度，恢复后直接在 `_keepalive_loop` 里发出陈旧 `_hold_q`，**阶跃照旧**——日志整场**没有 `[保活] 恢复前平滑
  过渡`**。时间线：`ready` 10:24:04→10:24:10 抬起；碰撞检查器 10:24:35→**10:24:39（4 s 饿死窗口＝现场看到的
  "放下"）**；validate 用窗口前读数过关（`drift=1.2e-05`），execute 重读得 **`max_drift=0.535 rad`（30.6°）**
  被拒——**pick 失败与巨响是同一根因的两个症状**，安全门正确拦下了基于陈旧种子的运动；随后线程恢复调度把
  30.6° 以单拍 30 Hz 发出＝**"立刻举起"与巨响**。**正确修法**：主因走 ②；另建议 `_keepalive_loop` 内按"距上次
  成功下发超过 N 个周期"自判并改走 ramp，才能覆盖饿死而非仅覆盖显式关闭。
...
- 下一步（按价值）：**① 消掉碰撞检查器的 4 s GIL 饿死窗口**（改为 arm_service 启动时一次性构建或移出该进程——已证实它同时造成下垂巨响、种子过期与 pick 失败，是当前最高价值的单点修复）→ **② `_keepalive_loop` 内自判调度断档并改走 ramp**（现补丁只守 `_set_keepalive` 翻转，覆盖不到饿死）→ **③ 用尺实测夹爪几何**（唯一能分离 TCP 误差与感知误差的手段，决定经验偏置是留是撤）→ ④ 抓取后段容差计入负载，消掉到位死区 → ⑤ 若嫌动作顿挫，把 `AIR_ROBOT_TRADITIONAL_POINT_DURATION_S` 调大或减少路点 → ⑥ 显式 bbox 被拒时回退自带检测器 → ⑦ 加抓取深度参数让 TCP 可落在瓶心之前 → ⑧ 姿态网格向有解一侧加密 → ⑨ 现场补量瓶身最宽处直径与瓶高 → ⑩ 多姿态手眼标定 → ⑪ 父项目改 `AIR_ROBOT_TRADITIONAL_GRASP_PYTHON` 上 GPU。
