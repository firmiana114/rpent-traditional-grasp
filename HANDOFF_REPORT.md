...
- **2026-08-02 11:47(失败)/11:50(成功) 对照，已同步到本地 `logs/`。结论：本项目规划器并没有
  "抓不到"，是目标当时刚好在可解边界之外。** 两次都是 `--air-robot-backend wuxi`，但**本项目服务
  收到的 `plan_pick` 请求数均为 0**（11:03 那次是 1），即 pick 后端已被换成旧的 Contact-GraspNet，
  `pick_object` 全程没经过本项目；本项目只承担 `search_object`（11:47 两次、11:50 七次）。
  - **11:50 成功那次，我把它抓取所用的那一帧原图离线喂给本项目规划器，规划同样成功**：可达性
    预检 `within_radius=True`、`required_base_advance=0`，TCP `[0.4778,0.0666,0.0564]`、左肩距
    **0.512 m**，24 个姿态候选中 `pitch_+10/+20/+30` **三个有解**，产出 38 点路径，最后只停在
    `motion_gated`（shadow 模式按设计不发运动）。**同一距离下两条链路都能规划。**
  - 11:03 失败那次左肩距 0.5422 m，11:50 是 0.5121 m，**只差 30 mm**，恰好跨过可解边界。
  - **姿态网格存在明显不对称**：只有**正俯角**（从略上方切入）有解，全部负俯角与全部偏航无解。
    24 个候选里有一半是无效开销，网格没有对准解实际存在的方向。
  - **底盘实际上没动**：11:50 里 3 次 0.1 m 前进原语都报成功，但前后两次 `search_object` 测得的
    瓶心只变了约 1.5–6 mm，`approach_object` 最终报 `max_iterations reached`。故成功**不能**归因
    于底盘前移。（该轮未记录底盘里程，此判断基于目标距离而非位姿读数。）
  - **两次的真实差别是"LLM 有没有让本项目的检测器跑"，不是两套抓取算法之争。** Contact-GraspNet
    自带检测器在两次的首次 `pick_object`（不带 bbox）都返回 `detect_failed`。11:50 的 LLM 随后调用
    了**不带 bbox** 的 `search_object`，本项目 YOLO-World 每次都正确定位（置信度 0.85–0.88），
    **最终促成抓取成功的 bbox `[203,194,252,299]` 正是本项目给出的**；11:47 的 LLM 则从头到尾只
    喂自己目测的框，一次都没调过不带 bbox 的 `search_object`。
  - **11:47 崩溃链条**：LLM 目测的 `[268,204,312,294]` 落在**空台面**上（已叠图确认），几何门禁按
    设计拒绝（`瓶径越界 0.1608m`、第二次 `商标带深度离散过大 mad=0.0284m`），但本项目是**抛异常**
    而非回退，`approach_object` 连带失败，重试门禁锁死。**显式 bbox 被拒时应回退到自带检测器**——
    这是本轮价值最高的一条，直接决定整轮能否继续。
...
- **2026-08-02 11:03 现场运行（旧标定生效后的第一次，仍失败；是三次里唯一由本项目承担 pick 的）**，
  ...
  - **瓶径真值存疑，暂缓**（用户 2026-08-02 决定）：现场量得**瓶底** 55.0 mm，且称昨今为同一瓶
    （与此前"62 mm"的说法冲突）。本项目估计的是**中部商标带**（掩码显示中部 38 px、瓶底 33 px，
    下收 13%），用同一深度图量瓶底得 57.9 mm。**要定案需现场补量瓶身最宽处直径与瓶高**；在此
    之前不要用瓶径反推深度尺度。
