...
- **"TRAC-IK chain 与 Pinocchio FK 差 11.3 cm"的外部结论已证伪**（2026-08-02 复核）。本链 FK 在 q=0 得
  `[0.353953,0.148633,0.051225]`，与对方算的 TRAC-IK 值**逐位一致**；差额可完全分解为两项**定义差**，
  非几何缺陷：① TCP 不同，`G1_29_ArmIK` 的 `L_ee` 是 wrist+0.05 m 的遥操重定向内部帧，本项目用
  wrist+0.150216 m 的 Dex1-1 抓取中心，差 100.216 mm；② **根坐标系不同**，对方在 `pelvis`、本项目在
  `torso_link`，差额正是 URDF `waist_roll_joint` 原点 `[-0.0039635,0,0.044]`——本链 FK 加此偏移即得
  对方的 `[0.349990,0.148633,0.095225]`，**精确到 1e-6 m**。故"改写 `g1_trac_ik.cpp` 从 URDF 建链"
  不会改变任何数值。顺带复核外参：`thor_camera_to_body_legacy.json` 平移 `[0.0481,0.0296,0.4360]` 与
  URDF `d435_joint` 的 `[0.0576,0.0175,0.4299]` 只差 ~12 mm（若误用 pelvis 系会差 44 mm），旋转即
  URDF 名义俯仰，**确系 `torso_link` 系**。
...
- **三次运行的 pick 后端归属（命名极易混淆，务必先看这条）**：父项目 `wuxi_adapter.py:421`
  ...
  `"backend": "air_robot traditional method stack"`。判别只能靠**唯一串**计数（下列依次为
  `did not return a grasp message`/`contact_graspnet`/`required_base_advance_m`/收到 `plan_pick`）：
  **11:03 = 0/0/2/1 → 本项目 traditional-live**；11:47 = 2/0/0/0 → legacy，失败；
...
- **父项目从不记录当前 pick 后端模式**（`run.log`、`observe_scene` 的 `capabilities`/`roots` 里都没有），
  ...
  **建议父项目在 `wuxi_adapter._pick_backend_mode()` 落一条 INFO，并把 mode 加进 `observe_scene`
  输出。** 机制上无歧义：默认值是 `traditional-live`，进 legacy **必须显式 export**；分发是
  `if mode != "legacy"` 二选一，`traditional_grasp_backend.py` **不含任何回退 legacy 的路径**；错误串
  全仓库仅一处；`AIR_ROBOT_PICK_BACKEND` 未见于任何脚本或 `.bashrc`，故最可能是手工 export 后残留。
- **11:47/11:50 对照，已同步到本地 `logs/`。要点：这两次都不是本项目跑的 pick**，`pick_object` 全程走旧
  Contact-GraspNet；本项目只承担 `search_object`（11:47 两次、11:50 七次）。**因此现场尚未做过同场景的
  跨方法 A/B**；要做需在当前摆位下用 `AIR_ROBOT_PICK_BACKEND=traditional-live` 再跑一次。
  **结论：本项目规划器并没有"抓不到"，是 11:03 时目标刚好在可解边界之外。**
  - **离线公平对照：把 11:50 成功抓取所用的那一帧原图喂给本项目规划器，规划同样成功**——预检
    `within_radius=True`、`required_base_advance=0`，TCP `[0.4778,0.0666,0.0564]`、左肩距 **0.512 m**，
    24 个候选中 `pitch_+10/+20/+30` **三个有解**，产出 38 点路径，只停在 `motion_gated`。11:03 失败那次
    左肩距 0.5422 m，与 11:50 的 0.5121 m **只差 30 mm**，恰好跨界。
  - **MuJoCo 仿真复刻两个位置，结果与现场逐一吻合**（2026-08-02，`--rest-bottle-on-table`，左臂，命令见
    "常用命令"）：11:50 位 `[0.4778,0.0666,0.0564]` → **抓取成功**，两指垫双边接触 `water_bottle_body`，
    **物理抬升 91.4 mm**，41 点路径，产物 `outputs/sim_1150_success/`；11:03 位 `[0.5093,0.0856,0.0517]`
    → **失败**，`planning_failed / no continuous IK path`，0 点路径。**独立物理引擎给出同样的成败分界。**
    同一次运行里本项目 TRAC-IK 与 MuJoCo FK 交叉审计 641 采样**最大位置误差 4.0e-07 m**，两臂
    `passed`——第三次独立证伪上面的 FK 不一致结论。已知偏差：Oracle 把瓶心 z 估到 0.0839（实摆
    0.0564）**偏高 27.5 mm**，即"仿真瓶模型含瓶颈"，成败结论不受影响但不可当绝对精度用。
  - **看 viewer 时的"先动一下再停"不是异常**：开头是仿真脚本的 `prepare_fixed_ready_pose`（与瓶子位姿
    ...
    （24 个候选，可行的每个约 0.63 s 做 29 点连续逆解，被拒的约 0.02 s），物理不推进；最后才是 40 点
    抓取轨迹。肩后收的作用是把 IK 种子推离零位的近伸直分支。
  - **姿态网格明显不对称**：只有**正俯角**（从略上方切入）有解，负俯角与偏航全灭，一半候选是无效开销。
  - **底盘实际上没动**：11:50 里 3 次 0.1 m 前进原语都报成功，但前后两次 `search_object` 测得的瓶心只变了
    约 1.5–6 mm，`approach_object` 最终报 `max_iterations reached`，故成功**不能**归因于底盘前移。
  ...
  - **11:47 与 11:50 的真实差别是"LLM 有没有让本项目的检测器跑"。** Contact-GraspNet 自带检测器在两次
    ...
    `[203,194,252,299]` 正是本项目给出的**；11:47 的 LLM 则从头到尾只喂自己目测的框，那个
    `[268,204,312,294]` 落在**空台面**上（已叠图确认），几何门禁按设计拒绝（`瓶径越界 0.1608m`、
    第二次 `商标带深度离散过大 mad=0.0284m`），但本项目是**抛异常**而非回退，`approach_object` 连带
    失败，重试门禁锁死。**显式 bbox 被拒时应回退到自带检测器**。
  - 与 Contact-GraspNet 的结构差异：本项目要求 TCP **精确落在瓶心**、姿态取自**固定 ±30° 网格**，24 个全无解即整体失败；对方从 3267 点点云**生成连续候选**并排序（本次取 `candidate_id=2`）。
  - **三条改进（按价值排序）**：① 显式 bbox 被几何门禁拒绝时回退到自带检测器；② 允许 TCP 落在瓶心
    **之前**（抓取深度参数），回收 10–20 mm 即等量延长可达距离；③ 姿态网格向有解一侧加密。
