...
- **2026-08-02 11:47(失败)/11:50(成功) 对照，已同步到本地 `logs/`。结论：本项目规划器并没有
  "抓不到"，是目标当时刚好在可解边界之外。** 两次都是 `--air-robot-backend wuxi`，`pick` 走的
  都是旧的 Contact-GraspNet；本项目在两次里承担的是 `search_object`。
  ...
  - **11:47 整轮崩溃的直接原因是本项目的硬失败策略**：上层 LLM 目测给的 bbox `[268,204,312,294]`
    落在**空台面**上（已叠图确认），几何门禁按设计拒绝（`瓶径越界 0.1608m`、第二次
    `商标带深度离散过大 mad=0.0284m`），但本项目是**抛异常**而非回退。同一套自带检测器在 11:50
    的每一次调用都正确找到了瓶子（置信度 0.85–0.88）。**显式 bbox 被拒时应回退到自带检测器。**
  - 与 Contact-GraspNet 的结构差异：本项目要求 TCP **精确落在瓶心**、姿态取自**固定 ±30° 网格**，
    24 个全无解即整体失败；对方从 3267 点点云**生成连续候选**并排序（本次取 `candidate_id=2`）。
  - **由此得出的三条改进（按价值排序）**：① 允许 TCP 落在瓶心**之前**（增加抓取深度参数）——夹爪
    只需包住瓶子，回收 10–20 mm 就等量延长可达距离，足以覆盖 11:03 差的那 2 mm；② 显式 bbox 被
    几何门禁拒绝时回退到自带检测器；③ 姿态网格改为向有解一侧加密或从可行域采样。
...
- 下一步（按价值）：**加抓取深度参数让 TCP 可落在瓶心之前** → **显式 bbox 被拒时回退自带检测器**
  → 姿态网格向有解一侧加密 → 现场补量瓶身最宽处直径与瓶高 → 多姿态手眼标定 → 父项目改
  `AIR_ROBOT_TRADITIONAL_GRASP_PYTHON` 上 GPU → 清场急停后限速小步真机验证。
