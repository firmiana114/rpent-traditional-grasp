...
- **已切回旧标定**（2026-08-02，用户决定）。依据：0801 现场运行与本机复现**两次独立测量**都
  报瓶径 0.062 m，与真值分毫不差；旧标定把瓶心拉到左肩距 0.5918 m，距实测无解边界 0.573 m
  仅差 19 mm（新标定差 88 mm）。两份配置的标定与外参**成对**切回 legacy（外参必须同切，
  20260801 外参由新标定的矫正旋转 R1 复合而来）。**保留疑点**：桌面缓存图那组旧标定瓶径
  69.1 mm 偏 +11.5%，未解释。
- 判定过程留档：新标定 fx 比旧的高 **20.5%**——同一台相机 fx 不应变，说明**至少一次标定病态**
  （标定板在画面里只占 0.6–0.9%）。极线错位 4.4px→1.6px 只检验旋转、**不检验尺度**；棋盘也不
  能验尺度，它就是标定所用的板，属循环论证。
...
- 侧抓可解半径远严于杆长上界，且**强烈依赖种子姿态与俯角**：零关节种子跑桌面缓存图方向时右臂
  0.5197 m 无解、0.5118 m 首次可解；仿真准备姿态跑现场场景时左臂 0.5475 m 无解、0.5392 m 可解。
...
- **2026-08-02 11:03 现场运行（旧标定生效后的第一次，仍失败）**，已同步到本地
  ...
  - **待确认（决定性）**：今天这瓶水的实测直径。`D = w_px·B/d` 与焦距无关，掩码经放大目视
    检查是**贴合**的（38–39 px），所以若真值仍是 62 mm，则深度同比偏小 **5.3%**（真值约
    0.597 m，左肩距约 0.572 m，正好落在实测无解边界 0.573 m 上）；若这瓶本就是 58–59 mm，
    则旧标定尺度**得到独立验证**。两种结论方向完全相反，必须现场量一次。
...
- 下一步：**现场量今天这瓶水的直径**（决定尺度是否已正确）→ 多姿态手眼标定 → 父项目改
