...
- **11:47/11:50 对照，已同步到本地 `logs/`。要点：这两次都不是本项目跑的 pick**，`pick_object`
  ...
  - **离线公平对照（当前唯一的跨方法证据）：把 11:50 成功抓取所用的那一帧原图喂给本项目规划器，
    规划同样成功**——可达性预检 `within_radius=True`、`required_base_advance=0`，TCP
    `[0.4778,0.0666,0.0564]`、左肩距 **0.512 m**，24 个姿态候选中 `pitch_+10/+20/+30` **三个有解**，
    产出 38 点路径，最后只停在 `motion_gated`（shadow 模式按设计不发运动）。11:03 失败那次左肩距
    0.5422 m，与 11:50 的 0.5121 m **只差 30 mm**，恰好跨过可解边界。
